...
/**
 * RepeatedDate implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedDateVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedDateVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private DateVector values;
  private final FieldReader reader = new RepeatedDateReaderImpl(RepeatedDateVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedDateVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public DateVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedDateVector) to);
  }

  @Override
  public AddOrGetResult<DateVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<DateVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedDateVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedDateVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedDateVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedDateVector(field, allocator);
    }

    public TransferImpl(RepeatedDateVector to) {
      this.to = to;
    }

    @Override
    public RepeatedDateVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedDateVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedDateVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedDateVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<DateTime> getObject(int index) {
      ...
      final DateVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedDateHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, DateHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableDateHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedDateHolder h) {
      final DateHolder ih = new DateHolder();
      final DateVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, DateHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableDateHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, DateHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedDateHolder holder) {

      DateVector.Accessor accessor = holder.vector.getAccessor();
      DateHolder innerHolder = new DateHolder();
