...
/**
 * RepeatedDecimal38Dense implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedDecimal38DenseVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedDecimal38DenseVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private Decimal38DenseVector values;
  private final FieldReader reader = new RepeatedDecimal38DenseReaderImpl(RepeatedDecimal38DenseVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedDecimal38DenseVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public Decimal38DenseVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedDecimal38DenseVector) to);
  }

  @Override
  public AddOrGetResult<Decimal38DenseVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<Decimal38DenseVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedDecimal38DenseVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedDecimal38DenseVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedDecimal38DenseVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedDecimal38DenseVector(field, allocator);
    }

    public TransferImpl(RepeatedDecimal38DenseVector to) {
      this.to = to;
    }

    @Override
    public RepeatedDecimal38DenseVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedDecimal38DenseVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedDecimal38DenseVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedDecimal38DenseVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<BigDecimal> getObject(int index) {
      ...
      final Decimal38DenseVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedDecimal38DenseHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, Decimal38DenseHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableDecimal38DenseHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedDecimal38DenseHolder h) {
      final Decimal38DenseHolder ih = new Decimal38DenseHolder();
      final Decimal38DenseVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, Decimal38DenseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableDecimal38DenseHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, Decimal38DenseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedDecimal38DenseHolder holder) {

      Decimal38DenseVector.Accessor accessor = holder.vector.getAccessor();
      Decimal38DenseHolder innerHolder = new Decimal38DenseHolder();
