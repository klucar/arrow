...
/**
 * RepeatedBit implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedBitVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedBitVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private BitVector values;
  private final FieldReader reader = new RepeatedBitReaderImpl(RepeatedBitVector.this);
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedBitVector) to);
  }

  @Override
  public AddOrGetResult<BitVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<BitVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedBitVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedBitVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedBitVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedBitVector(field, allocator);
    }

    public TransferImpl(RepeatedBitVector to) {
      this.to = to;
    }

    @Override
    public RepeatedBitVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedBitVector.this);
    }
  }
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Boolean> getObject(int index) {
      final List<Boolean> vals = new JsonStringArrayList<>();
      ...
      final BitVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public Boolean getSingleObject(int index, int arrayIndex) {
    ...
    public void get(int index, RepeatedBitHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, BitHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableBitHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedBitHolder h) {
      final BitHolder ih = new BitHolder();
      final BitVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, BitHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableBitHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, BitHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedBitHolder holder) {

      BitVector.Accessor accessor = holder.vector.getAccessor();
      BitHolder innerHolder = new BitHolder();
