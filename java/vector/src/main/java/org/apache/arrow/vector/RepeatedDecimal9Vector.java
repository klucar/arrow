...
/**
 * RepeatedDecimal9 implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedDecimal9Vector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedDecimal9Vector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private Decimal9Vector values;
  private final FieldReader reader = new RepeatedDecimal9ReaderImpl(RepeatedDecimal9Vector.this);
  ...
  public AddOrGetResult<Decimal9Vector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<Decimal9Vector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedDecimal9Vector to) {
  ...
  private class TransferImpl implements TransferPair {
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedDecimal9Vector.this);
    }
  }
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<BigDecimal> getObject(int index) {
      ...
      final Decimal9Vector.Accessor valuesAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedDecimal9Holder h) {
      final Decimal9Holder ih = new Decimal9Holder();
      final Decimal9Vector.Accessor hVectorAccessor = h.vector.getAccessor();
