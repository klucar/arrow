...
/**
 * RepeatedVarChar implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedVarCharVector extends BaseRepeatedValueVector implements RepeatedVariableWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedVarCharVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private VarCharVector values;
  private final FieldReader reader = new RepeatedVarCharReaderImpl(RepeatedVarCharVector.this);
  ...
  public AddOrGetResult<VarCharVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<VarCharVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedVarCharVector to) {
  ...
  private class TransferImpl implements TransferPair {
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedVarCharVector.this);
    }
  }
...
//  }
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Text> getObject(int index) {
      final List<Text> vals = new JsonStringArrayList<>();
      ...
      final VarCharVector.Accessor valuesAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedVarCharHolder h) {
      final VarCharHolder ih = new VarCharHolder();
      final VarCharVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int arrayIndex, int start, int end, ArrowBuf buffer) {
      int nextOffset = offsets.getAccessor().get(arrayIndex+1);
      values.getMutator().setSafe(nextOffset, start, end, buffer);
