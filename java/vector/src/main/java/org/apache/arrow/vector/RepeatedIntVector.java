...
/**
 * RepeatedInt implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedIntVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedIntVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private IntVector values;
  private final FieldReader reader = new RepeatedIntReaderImpl(RepeatedIntVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedIntVector(MaterializedField field, BufferAllocator allocator) {
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedIntVector) to);
  }

  @Override
  public AddOrGetResult<IntVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<IntVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedIntVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedIntVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedIntVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedIntVector(field, allocator);
    }

    public TransferImpl(RepeatedIntVector to) {
      this.to = to;
    }

    @Override
    public RepeatedIntVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedIntVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedIntVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedIntVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Integer> getObject(int index) {
      final List<Integer> vals = new JsonStringArrayList<>();
      ...
      final IntVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedIntHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, IntHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableIntHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedIntHolder h) {
      final IntHolder ih = new IntHolder();
      final IntVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, IntHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableIntHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, IntHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedIntHolder holder) {

      IntVector.Accessor accessor = holder.vector.getAccessor();
      IntHolder innerHolder = new IntHolder();
