...
/**
 * RepeatedUInt8 implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedUInt8Vector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedUInt8Vector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private UInt8Vector values;
  private final FieldReader reader = new RepeatedUInt8ReaderImpl(RepeatedUInt8Vector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedUInt8Vector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public UInt8Vector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedUInt8Vector) to);
  }

  @Override
  public AddOrGetResult<UInt8Vector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<UInt8Vector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedUInt8Vector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedUInt8Vector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedUInt8Vector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedUInt8Vector(field, allocator);
    }

    public TransferImpl(RepeatedUInt8Vector to) {
      this.to = to;
    }

    @Override
    public RepeatedUInt8Vector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedUInt8Vector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedUInt8Vector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedUInt8Vector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Long> getObject(int index) {
      final List<Long> vals = new JsonStringArrayList<>();
      ...
      final UInt8Vector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedUInt8Holder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, UInt8Holder holder) {
    ...
    public void get(int index, int positionIndex, NullableUInt8Holder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedUInt8Holder h) {
      final UInt8Holder ih = new UInt8Holder();
      final UInt8Vector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void add(int index, RepeatedUInt8Holder holder) {
      ...
      UInt8Vector.Accessor accessor = holder.vector.getAccessor();
      UInt8Holder innerHolder = new UInt8Holder();
