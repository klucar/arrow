...
/**
 * Float8 implements a vector of fixed width values.  Elements in the vector are accessed
 ...
 *   The equivalent Java primitive is 'double'
 *
...
public final class Float8Vector extends BaseDataValueVector implements FixedWidthVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(Float8Vector.class);

  private final FieldReader reader = new Float8ReaderImpl(Float8Vector.this);
  ...
  public Float8Vector(MaterializedField field, BufferAllocator allocator) {
...
//    }
  ...
  public void copyFrom(int fromIndex, int thisIndex, Float8Vector from){
    ...
    data.setDouble(thisIndex * 8,
        from.data.getDouble(fromIndex * 8)
    );
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor {
    ...
    public double getPrimitiveObject(int index) {
      return get(index);
    }

    public void get(int index, Float8Holder holder){

      holder.value = data.getDouble(index * 8);
    }
 ...
 /**
  * Float8.Mutator implements a mutable vector of fixed width values.  Elements in the
  ...
  */
  public final class Mutator extends BaseDataValueVector.BaseMutator {
   ...
   protected void set(int index, Float8Holder holder){
     data.setDouble(index * 8, holder.value);
   }
   ...
   protected void set(int index, NullableFloat8Holder holder){
     data.setDouble(index * 8, holder.value);
   }
