...
/**
 * NullableUInt8 implements a vector of values which could be null.  Elements in the vector
...
public final class NullableUInt8Vector extends BaseDataValueVector implements FixedWidthVector, NullableVector{
  ...
  private final FieldReader reader = new NullableUInt8ReaderImpl(NullableUInt8Vector.this);
  ...
  private final UInt8Vector values = new UInt8Vector(field, allocator);
...
//  }
  ...
  private class TransferImpl implements TransferPair {
    ...
    public TransferImpl(MaterializedField field, BufferAllocator allocator){
      to = new NullableUInt8Vector(field, allocator);
    }
  ...
  public UInt8Vector convertToRequiredVector(){
    UInt8Vector v = new UInt8Vector(getField().getOtherNullableVersion(), allocator);
    if (v.data != null) {
      v.data.release(1);
    }
    v.data = values.data;
