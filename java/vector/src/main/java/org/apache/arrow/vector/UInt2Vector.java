...
/**
 * UInt2 implements a vector of fixed width values.  Elements in the vector are accessed
 ...
 *   The equivalent Java primitive is 'char'
 *
...
public final class UInt2Vector extends BaseDataValueVector implements FixedWidthVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(UInt2Vector.class);

  private final FieldReader reader = new UInt2ReaderImpl(UInt2Vector.this);
...
//    }
  ...
  public void copyFrom(int fromIndex, int thisIndex, UInt2Vector from){
    ...
    data.setChar(thisIndex * 2,
        from.data.getChar(fromIndex * 2)
    );
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor {
    ...
    public void get(int index, UInt2Holder holder){
      ...
      holder.value = data.getChar(index * 2);
    }
 ...
 /**
  * UInt2.Mutator implements a mutable vector of fixed width values.  Elements in the
  ...
  */
  public final class Mutator extends BaseDataValueVector.BaseMutator {
   ...
   protected void set(int index, UInt2Holder holder){
     data.setChar(index * 2, holder.value);
   }
   ...
   public void generateTestData(int size) {
     ...
     for(int i = 0; i < valueCount; i++, even = !even) {
       if(even){
         set(i, Character.MIN_VALUE);
       }else{
         set(i, Character.MAX_VALUE);
       }
     }
   }
