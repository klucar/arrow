...
/**
 * NullableDate implements a vector of values which could be null.  Elements in the vector
...
public final class NullableDateVector extends BaseDataValueVector implements FixedWidthVector, NullableVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(NullableDateVector.class);

  private final FieldReader reader = new NullableDateReaderImpl(NullableDateVector.this);
  ...
  private final DateVector values = new DateVector(field, allocator);

  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public NullableDateVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public DateVector getValuesVector() {
    return values;
  }
...
//  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((NullableDateVector) to);
  }

  public void transferTo(NullableDateVector target){
    bits.transferTo(target.bits);
    values.transferTo(target.values);
    clear();
  }

  public void splitAndTransferTo(int startIndex, int length, NullableDateVector target) {
    bits.splitAndTransferTo(startIndex, length, target.bits);
    values.splitAndTransferTo(startIndex, length, target.values);
  }

  private class TransferImpl implements TransferPair {
    NullableDateVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator){
      to = new NullableDateVector(field, allocator);
    }

    public TransferImpl(NullableDateVector to){
      this.to = to;
    }

    @Override
    public NullableDateVector getTo(){
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, NullableDateVector.this);
    }
  }
  ...
  public DateVector convertToRequiredVector(){
    DateVector v = new DateVector(getField().getOtherNullableVersion(), allocator);
    if (v.data != null) {
      v.data.release(1);
    }
    v.data = values.data;
  ...
  public void copyFrom(int fromIndex, int thisIndex, NullableDateVector from){
  ...
  public void copyFromSafe(int fromIndex, int thisIndex, NullableDateVector from){
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor  {
    ...
    final DateVector.Accessor vAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseDataValueVector.BaseMutator implements NullableVectorDefinitionSetter {
    ...
    public DateVector getVectorWithValues(){
      return values;
    }
    ...
    public void set(int index, long value) {
      setCount++;
      final DateVector.Mutator valuesMutator = values.getMutator();
    ...
    public void setSkipNull(int index, DateHolder holder){
      values.getMutator().set(index, holder);
    }

    public void setSkipNull(int index, NullableDateHolder holder){
      values.getMutator().set(index, holder);
    }


    public void set(int index, NullableDateHolder holder){
      final DateVector.Mutator valuesMutator = values.getMutator();
    ...
    public void set(int index, DateHolder holder){
      final DateVector.Mutator valuesMutator = values.getMutator();
      bits.getMutator().set(index, 1);
      valuesMutator.set(index, holder);
    ...
    public boolean isSafe(int outIndex) {
      return outIndex < NullableDateVector.this.getValueCapacity();
    }
    ...
    public void setSafe(int index, NullableDateHolder value) {
    ...
    public void setSafe(int index, DateHolder value) {
