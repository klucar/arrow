...
public final class RepeatedUInt4Vector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedUInt4Vector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private UInt4Vector values;
  private final FieldReader reader = new RepeatedUInt4ReaderImpl(RepeatedUInt4Vector.this);
  ...
  @Override
  public UInt4Vector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedUInt4Vector) to);
  }

  @Override
  public AddOrGetResult<UInt4Vector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<UInt4Vector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedUInt4Vector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedUInt4Vector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedUInt4Vector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedUInt4Vector(field, allocator);
    }

    public TransferImpl(RepeatedUInt4Vector to) {
      this.to = to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedUInt4Vector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedUInt4Vector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedUInt4Vector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Integer> getObject(int index) {
      ...
      final UInt4Vector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, int positionIndex, UInt4Holder holder) {
    ...
    public void get(int index, int positionIndex, NullableUInt4Holder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedUInt4Holder h) {
      final UInt4Holder ih = new UInt4Holder();
      final UInt4Vector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, UInt4Holder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableUInt4Holder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
