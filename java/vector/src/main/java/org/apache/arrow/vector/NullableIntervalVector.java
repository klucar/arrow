...
/**
 * NullableInterval implements a vector of values which could be null.  Elements in the vector
...
public final class NullableIntervalVector extends BaseDataValueVector implements FixedWidthVector, NullableVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(NullableIntervalVector.class);

  private final FieldReader reader = new NullableIntervalReaderImpl(NullableIntervalVector.this);
  ...
  private final IntervalVector values = new IntervalVector(field, allocator);

  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public NullableIntervalVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public IntervalVector getValuesVector() {
    return values;
  }
...
//  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((NullableIntervalVector) to);
  }

  public void transferTo(NullableIntervalVector target){
    bits.transferTo(target.bits);
    values.transferTo(target.values);
    clear();
  }

  public void splitAndTransferTo(int startIndex, int length, NullableIntervalVector target) {
    bits.splitAndTransferTo(startIndex, length, target.bits);
    values.splitAndTransferTo(startIndex, length, target.values);
  }

  private class TransferImpl implements TransferPair {
    NullableIntervalVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator){
      to = new NullableIntervalVector(field, allocator);
    }

    public TransferImpl(NullableIntervalVector to){
      this.to = to;
    }

    @Override
    public NullableIntervalVector getTo(){
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, NullableIntervalVector.this);
    }
  }
  ...
  public IntervalVector convertToRequiredVector(){
    IntervalVector v = new IntervalVector(getField().getOtherNullableVersion(), allocator);
    if (v.data != null) {
      v.data.release(1);
    }
    v.data = values.data;
  ...
  public void copyFrom(int fromIndex, int thisIndex, NullableIntervalVector from){
  ...
  public void copyFromSafe(int fromIndex, int thisIndex, NullableIntervalVector from){
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor  {
    ...
    final IntervalVector.Accessor vAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseDataValueVector.BaseMutator implements NullableVectorDefinitionSetter {
    ...
    public IntervalVector getVectorWithValues(){
      return values;
    }
    ...
    public void set(int index, ArrowBuf value) {
      setCount++;
      final IntervalVector.Mutator valuesMutator = values.getMutator();
    ...
    public void setSkipNull(int index, IntervalHolder holder){
      values.getMutator().set(index, holder);
    }

    public void setSkipNull(int index, NullableIntervalHolder holder){
      values.getMutator().set(index, holder);
    }


    public void set(int index, NullableIntervalHolder holder){
      final IntervalVector.Mutator valuesMutator = values.getMutator();
    ...
    public void set(int index, IntervalHolder holder){
      final IntervalVector.Mutator valuesMutator = values.getMutator();
      bits.getMutator().set(index, 1);
      valuesMutator.set(index, holder);
    ...
    public boolean isSafe(int outIndex) {
      return outIndex < NullableIntervalVector.this.getValueCapacity();
    }

    public void set(int index, int isSet, int monthsField, int daysField, int millisecondsField ){
      final IntervalVector.Mutator valuesMutator = values.getMutator();
      bits.getMutator().set(index, isSet);
      valuesMutator.set(index, monthsField, daysField, millisecondsField);
      
    }

    public void setSafe(int index, int isSet, int monthsField, int daysField, int millisecondsField ) {

      bits.getMutator().setSafe(index, isSet);
      values.getMutator().setSafe(index, monthsField, daysField, millisecondsField);
      setCount++;
      
    }


    public void setSafe(int index, NullableIntervalHolder value) {
    ...
    public void setSafe(int index, IntervalHolder value) {
