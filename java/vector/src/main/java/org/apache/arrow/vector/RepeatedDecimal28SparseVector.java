...
/**
 * RepeatedDecimal28Sparse implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedDecimal28SparseVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedDecimal28SparseVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private Decimal28SparseVector values;
  private final FieldReader reader = new RepeatedDecimal28SparseReaderImpl(RepeatedDecimal28SparseVector.this);
  ...
  @Override
  public Decimal28SparseVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedDecimal28SparseVector) to);
  }

  @Override
  public AddOrGetResult<Decimal28SparseVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<Decimal28SparseVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedDecimal28SparseVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedDecimal28SparseVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedDecimal28SparseVector(field, allocator);
    }

    public TransferImpl(RepeatedDecimal28SparseVector to) {
      this.to = to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedDecimal28SparseVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedDecimal28SparseVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedDecimal28SparseVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<BigDecimal> getObject(int index) {
      ...
      final Decimal28SparseVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedDecimal28SparseHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, Decimal28SparseHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableDecimal28SparseHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedDecimal28SparseHolder h) {
      final Decimal28SparseHolder ih = new Decimal28SparseHolder();
      final Decimal28SparseVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, Decimal28SparseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableDecimal28SparseHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, Decimal28SparseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedDecimal28SparseHolder holder) {

      Decimal28SparseVector.Accessor accessor = holder.vector.getAccessor();
      Decimal28SparseHolder innerHolder = new Decimal28SparseHolder();
