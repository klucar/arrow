...
/**
 * RepeatedSmallInt implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedSmallIntVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedSmallIntVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private SmallIntVector values;
  private final FieldReader reader = new RepeatedSmallIntReaderImpl(RepeatedSmallIntVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedSmallIntVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public SmallIntVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedSmallIntVector) to);
  }

  @Override
  public AddOrGetResult<SmallIntVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<SmallIntVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedSmallIntVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedSmallIntVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedSmallIntVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedSmallIntVector(field, allocator);
    }

    public TransferImpl(RepeatedSmallIntVector to) {
      this.to = to;
    }

    @Override
    public RepeatedSmallIntVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedSmallIntVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedSmallIntVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedSmallIntVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    @Override
    public List<Short> getObject(int index) {
      final List<Short> vals = new JsonStringArrayList<>();
      ...
      final SmallIntVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public Short getSingleObject(int index, int arrayIndex) {
    ...
    public void get(int index, RepeatedSmallIntHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, SmallIntHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableSmallIntHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void addSafe(int index, short srcValue) {
    ...
    public void setSafe(int index, RepeatedSmallIntHolder h) {
      final SmallIntHolder ih = new SmallIntHolder();
      final SmallIntVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, SmallIntHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableSmallIntHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, SmallIntHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedSmallIntHolder holder) {

      SmallIntVector.Accessor accessor = holder.vector.getAccessor();
      SmallIntHolder innerHolder = new SmallIntHolder();
