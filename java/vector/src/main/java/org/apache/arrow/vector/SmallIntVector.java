...
/**
 * SmallInt implements a vector of fixed width values.  Elements in the vector are accessed
 ...
 *   The width of each element is 2 byte(s)
...
public final class SmallIntVector extends BaseDataValueVector implements FixedWidthVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SmallIntVector.class);

  private final FieldReader reader = new SmallIntReaderImpl(SmallIntVector.this);
  private final Accessor accessor = new Accessor();
  private final Mutator mutator = new Mutator();

  private int allocationSizeInBytes = INITIAL_VALUE_ALLOCATION * 2;
  ...
  public int getValueCapacity(){
    return (int) (data.capacity() *1.0 / 2);
  }
  ...
  public void setInitialCapacity(final int valueCount) {
    final long size = 1L * valueCount * 2;
...
//    final int expectedLength = valueCount * 2;
...
//    }
  ...
  public void splitAndTransferTo(int startIndex, int length, SmallIntVector target) {
    final int startPoint = startIndex * 2;
    final int sliceLength = length * 2;
    target.clear();
  ...
  public void copyFrom(int fromIndex, int thisIndex, SmallIntVector from){
    ...
    data.setShort(thisIndex * 2,
        from.data.getShort(fromIndex * 2)
    );
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor {
    ...
    public short getPrimitiveObject(int index) {
      return get(index);
    }

    public void get(int index, SmallIntHolder holder){

      holder.value = data.getShort(index * 2);
    }
 ...
 /**
  * SmallInt.Mutator implements a mutable vector of fixed width values.  Elements in the
  ...
  */
  public final class Mutator extends BaseDataValueVector.BaseMutator {
   ...
   protected void set(int index, SmallIntHolder holder){
     data.setShort(index * 2, holder.value);
   }
   ...
   protected void set(int index, NullableSmallIntHolder holder){
     data.setShort(index * 2, holder.value);
   }
   ...
   public void generateTestData(int size) {
     ...
     for(int i = 0; i < valueCount; i++, even = !even) {
       if(even){
         set(i, Short.MIN_VALUE);
       }else{
         set(i, Short.MAX_VALUE);
       }
     }
   }
