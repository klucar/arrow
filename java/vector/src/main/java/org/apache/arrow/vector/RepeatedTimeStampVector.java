...
/**
 * RepeatedTimeStamp implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedTimeStampVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedTimeStampVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private TimeStampVector values;
  private final FieldReader reader = new RepeatedTimeStampReaderImpl(RepeatedTimeStampVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedTimeStampVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public TimeStampVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedTimeStampVector) to);
  }

  @Override
  public AddOrGetResult<TimeStampVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<TimeStampVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedTimeStampVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedTimeStampVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedTimeStampVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedTimeStampVector(field, allocator);
    }

    public TransferImpl(RepeatedTimeStampVector to) {
      this.to = to;
    }

    @Override
    public RepeatedTimeStampVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedTimeStampVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedTimeStampVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedTimeStampVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<DateTime> getObject(int index) {
      final List<DateTime> vals = new JsonStringArrayList<>();
      ...
      final TimeStampVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public DateTime getSingleObject(int index, int arrayIndex) {
    ...
    public void get(int index, RepeatedTimeStampHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, TimeStampHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableTimeStampHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedTimeStampHolder h) {
      final TimeStampHolder ih = new TimeStampHolder();
      final TimeStampVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, TimeStampHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableTimeStampHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, TimeStampHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedTimeStampHolder holder) {

      TimeStampVector.Accessor accessor = holder.vector.getAccessor();
      TimeStampHolder innerHolder = new TimeStampHolder();
