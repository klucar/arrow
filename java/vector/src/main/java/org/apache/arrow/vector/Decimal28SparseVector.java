...
/**
 * Decimal28Sparse implements a vector of fixed width values.  Elements in the vector are accessed
 ...
 *   The width of each element is 20 byte(s)
...
public final class Decimal28SparseVector extends BaseDataValueVector implements FixedWidthVector{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(Decimal28SparseVector.class);

  private final FieldReader reader = new Decimal28SparseReaderImpl(Decimal28SparseVector.this);
  private final Accessor accessor = new Accessor();
  private final Mutator mutator = new Mutator();

  private int allocationSizeInBytes = INITIAL_VALUE_ALLOCATION * 20;
  ...
  public int getValueCapacity(){
    return (int) (data.capacity() *1.0 / 20);
  }
  ...
  public void setInitialCapacity(final int valueCount) {
    final long size = 1L * valueCount * 20;
...
//    final int expectedLength = valueCount * 20;
...
//    }
  ...
  public void splitAndTransferTo(int startIndex, int length, Decimal28SparseVector target) {
    final int startPoint = startIndex * 20;
    final int sliceLength = length * 20;
    target.clear();
  ...
  private class TransferImpl implements TransferPair{
    ...
    public TransferImpl(MaterializedField field, BufferAllocator allocator){
      to = new Decimal28SparseVector(field, allocator);
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, Decimal28SparseVector.this);
    }
  }

  public void copyFrom(int fromIndex, int thisIndex, Decimal28SparseVector from){
    from.data.getBytes(fromIndex * 20, data, thisIndex * 20, 20);
  ...
  public final class Accessor extends BaseDataValueVector.BaseAccessor {
    ...
    public void get(int index, NullableDecimal28SparseHolder holder) {
      ...
      return org.apache.arrow.vector.util.DecimalUtility.getBigDecimalFromSparse(data, index * 20, 5, getField().getScale());
    }
 ...
 /**
  * Decimal28Sparse.Mutator implements a mutable vector of fixed width values.  Elements in the
  ...
  */
  public final class Mutator extends BaseDataValueVector.BaseMutator {
   ...
   public void set(int index, ArrowBuf value) {
     data.setBytes(index * 20, value, 0, 20);
   }
   ...
   public void set(int index, int start, ArrowBuf buffer){
     data.setBytes(index * 20, buffer, start, 20);
   }
   ...
   public void setValueCount(int valueCount) {
     ...
     data.writerIndex(valueCount * 20);
   }
 }
}
