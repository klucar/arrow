...
/**
 * RepeatedFloat4 implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedFloat4Vector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedFloat4Vector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private Float4Vector values;
  private final FieldReader reader = new RepeatedFloat4ReaderImpl(RepeatedFloat4Vector.this);
  ...
  public AddOrGetResult<Float4Vector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<Float4Vector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedFloat4Vector to) {
  ...
  private class TransferImpl implements TransferPair {
    ...
    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedFloat4Vector(field, allocator);
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedFloat4Vector.this);
    }
  }
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Float> getObject(int index) {
      final List<Float> vals = new JsonStringArrayList<>();
      ...
      final Float4Vector.Accessor valuesAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedFloat4Holder h) {
      final Float4Holder ih = new Float4Holder();
      final Float4Vector.Accessor hVectorAccessor = h.vector.getAccessor();
