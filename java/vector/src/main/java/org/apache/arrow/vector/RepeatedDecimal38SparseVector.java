...
/**
 * RepeatedDecimal38Sparse implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedDecimal38SparseVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedDecimal38SparseVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private Decimal38SparseVector values;
  private final FieldReader reader = new RepeatedDecimal38SparseReaderImpl(RepeatedDecimal38SparseVector.this);
  private final Mutator mutator = new Mutator();
  private final Accessor accessor = new Accessor();

  public RepeatedDecimal38SparseVector(MaterializedField field, BufferAllocator allocator) {
  ...
  @Override
  public Decimal38SparseVector getDataVector() {
    return values;
  }
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((RepeatedDecimal38SparseVector) to);
  }

  @Override
  public AddOrGetResult<Decimal38SparseVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<Decimal38SparseVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void transferTo(RepeatedDecimal38SparseVector target) {
    target.clear();
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedDecimal38SparseVector to) {
  ...
  private class TransferImpl implements TransferPair {
    final RepeatedDecimal38SparseVector to;

    public TransferImpl(MaterializedField field, BufferAllocator allocator) {
      this.to = new RepeatedDecimal38SparseVector(field, allocator);
    }

    public TransferImpl(RepeatedDecimal38SparseVector to) {
      this.to = to;
    }

    @Override
    public RepeatedDecimal38SparseVector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedDecimal38SparseVector.this);
    }
  }

    public void copyFrom(int inIndex, int outIndex, RepeatedDecimal38SparseVector v) {
    ...
    public void copyFromSafe(int inIndex, int outIndex, RepeatedDecimal38SparseVector v) {
      final Accessor vAccessor = v.getAccessor();
      final int count = vAccessor.getInnerValueCountAt(inIndex);
      mutator.startNewValue(outIndex);
      for (int i = 0; i < count; i++) {
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<BigDecimal> getObject(int index) {
      final List<BigDecimal> vals = new JsonStringArrayList<>();
      ...
      final Decimal38SparseVector.Accessor valuesAccessor = values.getAccessor();
    ...
    public void get(int index, RepeatedDecimal38SparseHolder holder) {
      holder.start = offsets.getAccessor().get(index);
      holder.end =  offsets.getAccessor().get(index+1);
      holder.vector = values;
    }

    public void get(int index, int positionIndex, Decimal38SparseHolder holder) {
    ...
    public void get(int index, int positionIndex, NullableDecimal38SparseHolder holder) {
      final int offset = offsets.getAccessor().get(index);
      assert offset >= 0;
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
     ...
     */
    public void add(int index, ArrowBuf value) {
    ...
    public void addSafe(int index, ArrowBuf srcValue) {
    ...
    public void setSafe(int index, RepeatedDecimal38SparseHolder h) {
      final Decimal38SparseHolder ih = new Decimal38SparseHolder();
      final Decimal38SparseVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int index, Decimal38SparseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }

    public void addSafe(int index, NullableDecimal38SparseHolder holder) {
      final int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().setSafe(nextOffset, holder);
      offsets.getMutator().setSafe(index+1, nextOffset+1);
    }
    ...
    protected void add(int index, Decimal38SparseHolder holder) {
      int nextOffset = offsets.getAccessor().get(index+1);
      values.getMutator().set(nextOffset, holder);
      offsets.getMutator().set(index+1, nextOffset+1);
    }

    public void add(int index, RepeatedDecimal38SparseHolder holder) {

      Decimal38SparseVector.Accessor accessor = holder.vector.getAccessor();
      Decimal38SparseHolder innerHolder = new Decimal38SparseHolder();
