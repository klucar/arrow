...
/**
 * RepeatedIntervalDay implements a vector with multple values per row (e.g. JSON array or
...
public final class RepeatedIntervalDayVector extends BaseRepeatedValueVector implements RepeatedFixedWidthVectorLike {
  //private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RepeatedIntervalDayVector.class);

  // we maintain local reference to concrete vector type for performance reasons.
  private IntervalDayVector values;
  private final FieldReader reader = new RepeatedIntervalDayReaderImpl(RepeatedIntervalDayVector.this);
  ...
  public AddOrGetResult<IntervalDayVector> addOrGetVector(VectorDescriptor descriptor) {
    final AddOrGetResult<IntervalDayVector> result = super.addOrGetVector(descriptor);
    if (result.isCreated()) {
  ...
  public void splitAndTransferTo(final int startIndex, final int groups, RepeatedIntervalDayVector to) {
  ...
  private class TransferImpl implements TransferPair {
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, RepeatedIntervalDayVector.this);
    }
  }
  ...
  public final class Accessor extends BaseRepeatedValueVector.BaseRepeatedAccessor {
    ...
    public List<Period> getObject(int index) {
      final List<Period> vals = new JsonStringArrayList<>();
      ...
      final IntervalDayVector.Accessor valuesAccessor = values.getAccessor();
  ...
  public final class Mutator extends BaseRepeatedValueVector.BaseRepeatedMutator implements RepeatedMutator {
    ...
    public void setSafe(int index, RepeatedIntervalDayHolder h) {
      final IntervalDayHolder ih = new IntervalDayHolder();
      final IntervalDayVector.Accessor hVectorAccessor = h.vector.getAccessor();
    ...
    public void addSafe(int arrayIndex, int days, int milliseconds) {
      int nextOffset = offsets.getAccessor().get(arrayIndex+1);
      values.getMutator().setSafe(nextOffset, days, milliseconds);
